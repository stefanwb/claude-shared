...
| `--registry` | Surface host-native private package-registry config so in-container `uv` / `pnpm` / pip installs resolve against your private feed (CodeArtifact, Artifactory, Nexus, …). Mounts `~/.npmrc`, `~/.netrc`, `~/.config/uv/uv.toml`, and the platform-appropriate `pip.conf` read-only when present, and forwards `npm_config_registry` / `NODE_AUTH_TOKEN` / `NPM_TOKEN` / `UV_INDEX_URL` / `UV_DEFAULT_INDEX` / `UV_INDEX_*` / `PIP_INDEX_URL` / `PIP_EXTRA_INDEX_URL` / `PIPENV_PYPI_MIRROR` when set. Runtime-only; the image build is unaffected. See [Private package registries](#private-package-registries). |
...
> **⚠️ Use with care — this uses whole-file mounts, not just the registry line.** Everything in the mounted files becomes readable inside the container, including credentials and settings that have nothing to do with your package feed. Inspect these files before using:
>
> - **`~/.netrc` is the riskiest.** It's a generic, machine-keyed credential store — it can hold logins/passwords for arbitrary hosts (git remotes, FTP, any netrc-aware tool), and `--registry` forwards *all* of them, not just your registry's. If you keep unrelated secrets in `~/.netrc`, do not pass `--registry`, or point uv at a dedicated netrc with `UV_NETRC=/path/to/registry-only.netrc` and keep that file scoped.
...
>
> The forwarded *env vars* are tightly scoped (named individually), so the over-share is specific to the file mounts. To minimise exposure, prefer the env-var channel, or keep registry-only config files, and remember the container has full network egress (see [Threat model](#threat-model)).
...
|---------|-----------|----|--------------|
| Config file (`:ro` mount) | `~/.npmrc` | `~/.config/uv/uv.toml`, `~/.netrc` | platform `pip.conf` (macOS: `~/Library/Application Support/pip/pip.conf`, Linux: `~/.config/pip/pip.conf`), `~/.netrc` |
| Env vars (forwarded when set) | `npm_config_registry`, `NPM_CONFIG_REGISTRY`, `NODE_AUTH_TOKEN`, `NPM_TOKEN` | `UV_INDEX_URL`, `UV_DEFAULT_INDEX`, `UV_EXTRA_INDEX_URL`, `UV_INDEX`, `UV_NETRC`, `UV_KEYRING_PROVIDER`, and any `UV_INDEX_<NAME>_USERNAME` / `_PASSWORD` | `PIP_INDEX_URL`, `PIP_EXTRA_INDEX_URL`, `PIP_TRUSTED_HOST`, `PIPENV_PYPI_MIRROR` |
...
- **Exposed (per session):** your passed workspaces are read-write (unless `--ro`); host credentials when opted in — short-lived AWS SSO bearer tokens (`~/.aws/sso/cache`), the glab config token, `~/.terraform.d/credentials.tfrc.json`, and `GH_TOKEN` / `GITLAB_TOKEN` / `TF_TOKEN_app_terraform_io` / `AWS_*` env vars are all readable inside the container; under `--registry`, the **full contents** of mounted `~/.npmrc` / `~/.netrc` / `pip.conf` (and forwarded `*_TOKEN` / `UV_INDEX_*_PASSWORD` env) are readable — and those files can hold credentials for hosts well beyond your registry (`~/.netrc` especially), see [Private package registries](#private-package-registries); full outbound network with no egress filtering.
